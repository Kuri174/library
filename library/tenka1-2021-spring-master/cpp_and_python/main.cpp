...
struct Bot {
  ...
  Bot() {
    ...
    for (int i = 0; i < master_data.num_agent; ++i) {
      agent_last_point[i] = {(int)game_info.agent[i].move.back().x,
                             (int)game_info.agent[i].move.back().y};
      set_move_point(i);
    }
  }
  // struct Task {
  ...
  const Task& choice_task() {
    ...
    vector<int> Id(6, 0), Total(6, 1e9);
    for (int i = 0; i < game_info.task.size(); i++) {
      auto t = game_info.task[i];
      for (int j = 5; j <= 10; j++) {
        if (t.weight == 10000 and t.s.size() == j) {
          if (Total[j - 5] > t.total) {
            Total[j - 5] = t.total;
            Id[j - 5] = i;
          }
        }
    ...
    if (Total[0] < 100) return game_info.task[Id[0]];
    for (int i = 1; i < 6; i++) {
      Total[i] = 1.0 * Total[i] * (100 + i * 5);
    }
    int id = 0;
    double res = -1e9;
    for (int i = 0; i < 6; i++) {
      double score = 10000.0 / Total[i];
      if (res < score) {
        res = score;
        id = i;
      }
    }
    return game_info.task[Id[id]];
  }
  ...
  int get_now_game_time_ms() {
    auto t = chrono::duration_cast<std::chrono::milliseconds>(
  ...
  Move move_next(int index) {
    ...
    cerr << "Agent#" << index + 1 << " move_next to (" << x << ", " << y << ")"
         << endl;
  ...
  void solve() {
    for (;;) {
      ...
      if (next_call_game_info_time_ms < now_game_time_ms) {
        ...
        game_info = call_game();
        next_call_game_info_time_ms =
            get_now_game_time_ms() + GAME_INFO_SLEEP_TIME;
